...
2. aws rds modify-db-instance --region <region-name> --db-instanceidentifier <db-name> --multi-az --apply-immediately
3. Confirm that the Multi-AZ deployment is enabled by running the following command:
4. aws rds describe-db-instances --region <region-name> --db-instanceidentifier <db-name> --query 'DBInstances[*] MultiAZ'
- The output should return True, indicating that Multi-AZ is enabled.
5. Repeat the procedure for other instances as necessary
