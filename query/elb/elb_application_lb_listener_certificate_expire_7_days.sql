select
  ...
  case
    when date(not_after) - date(current_date) >= 7 then l.title || ' certificate set to expire in ' || extract(day from not_after - current_date) || ' days.' else l.title || ' certificate expired' || to_char(not_after, 'DD-Mon-YYYY') || '.'
  end as reason,
