...
select
  ...
  case
    ...
    when not include_global_service_events then title || ' not records global service events.'
    ...
    when not (jsonb_array_length(event_selectors) = 1 and event_selectors @> '[{"ReadWriteType":"All"}]') then title || ' not records events for both reads and writes.'
    when not (event_selectors @> '[{"IncludeManagementEvents":true}]') then title || ' not records management events.'
