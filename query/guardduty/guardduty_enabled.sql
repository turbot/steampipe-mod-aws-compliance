select
  ...
  case
    when a.region = any ( ARRAY ['af-south-1', 'ap-northeast-3', 'ap-southeast-3', 'eu-south-1', 'cn-north-1', 'cn-northwest-1', 'me-south-1', 'us-gov-east-1'] ) then 'Region not supported.'
    when status is null then 'No guardduty detector found.'
    when status = 'ENABLED' then r.title || ' enabled.'
