...
## 1. query: iam_role_cross_account_read_only_access_policy
  ...
  - Lateral Join: The join lateral ensures we only process roles with the policy, and stops at the first match. here we are using limit 1 , so performance should be good
...
## 3. query: vpc_security_group_restricted_common_ports
...
#### Key Points:
- Array-Based Port Definition, JOIN Instead of Multiple OR Conditions
- OLD: O(n × m) where n = rules, m = port conditions,  NEW: O(n × p) where n = rules, p = ports
- Each rule joined with port array (more efficient)

## 4. query: vpc_gateway_endpoint_restrict_public_access
...
## 5. query: vpc_configured_to_use_vpc_endpoints
...
## 6. query: vpc_network_acl_remote_administration
...
## 7. query: vpc_security_group_restrict_ingress_tcp_udp_all
...
## 8. query: vpc_security_group_restrict_ingress_tcp_udp_all
...
## 9. query: vpc_security_group_restrict_ingress_common_ports_all
...
## 10. query: vpc_security_group_associated_to_eni
...
## 11. query: vpc_security_group_allows_ingress_to_oracle_ports
...
## 12. query: ec2_instance_attached_ebs_volume_delete_on_termination_enabled
...
## 13. query: ec2_ami_ebs_encryption_enabled
...
## 14. query: log_metric_filter_root_login

  #### Old: Time: 7.5s. Rows returned: 1. Rows fetched: 65. Hydrate calls: 196. Scans: 2.
  ...
  #### New Time: 3.4s. Rows returned: 1. Rows fetched: 66. Hydrate calls: 196. Scans: 2.
