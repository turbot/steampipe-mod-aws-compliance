...
query "guardduty_enabled" {
  sql = <<-EOQ
    select
      ...
      case
        when status = 'ENABLED' and master_account ->> 'AccountId' is null then region || ' detector ' || title || ' enabled.'
        when status = 'ENABLED' and master_account ->> 'AccountId' is not null then region || ' detector ' || title || ' is managed by account ' || (master_account ->> 'AccountId') ||  ' via delegated admin.'
        else region || ' detector ' || title || ' disabled.'
      end as reason
