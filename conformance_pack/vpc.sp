...
control "vpc_security_group_restrict_ingress_tcp_udp_all" {
  title       = "VPC security groups should restrict inbound TCP and UDP access from 0.0.0.0/0"
...
control "vpc_security_group_restrict_inbound_common_ports_all" {
  title       = "VPC security groups should restrict inbound access on ports 20, 21, 22, 3306, 3389, 4333 from 0.0.0.0/0"
  description = "Manage access to resources in the AWS Cloud by ensuring common ports are restricted on Amazon Elastic Compute Cloud (Amazon EC2) security groups."
  sql         = query.vpc_security_group_restrict_inbound_common_ports_all.sql
  tags        = local.conformance_pack_vpc_common_tags
}

control "vpc_security_group_restrict_inbound_ssh_all" {
  title       = "VPC security groups should restrict inbound SSH access from 0.0.0.0/0"
  ...
  sql         = query.vpc_security_group_restrict_inbound_ssh_all.sql
  tags        = local.conformance_pack_vpc_common_tags
}
